...
class FamilyManagerPanel:
    ...
    def update_stats(self):
        ...
        if self.app_instance and hasattr(self.app_instance, 'persistence_service'):
            save_info = self.app_instance.persistence_service.get_save_info()
            if save_info['exists']:
                stats_text += "💾 ÚLTIMO GUARDADO:\n"
                stats_text += f"📅 {save_info['saved_at_str']}"
            else:
                stats_text += "💾 ESTADO: Sin guardar"
    ...
    def manual_save(self):
        ...
        if self.app_instance:
            ...
            if success:
                ...
                # Actualizar estadísticas para mostrar nueva fecha de guardado
                self.update_stats()
            else:
    ...
    def create_backup(self):
        ...
        if self.app_instance:
            ...
            messagebox.showwarning("Advertencia", "No se puede crear backup: referencia a la aplicación no disponible")
